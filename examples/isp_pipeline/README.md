...
flowchart LR
    ...
    subgraph WL["7 worker 实例（PAL 线程，硬件行为模拟；各持一个心跳槽）"]
...
7 个非 AO worker 线程各注册一个心跳槽；`WorkerBase` / `UsbDmaWorker` 的内层等待
循环与 `IrscWorker` 的产帧循环套 `BeatLoop` 骨架，main.cpp 的 8 处等待循环套
`ScanLoop` 骨架。两条横切不变式由骨架拥有，循环体无法跳过。判定只到"每槽上次
打点时间 + 超时"，不接降级与喂狗（见 `docs/design_isp_pipeline_heartbeat_zh.md`）。
