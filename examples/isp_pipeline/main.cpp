...
// ---------------------------------------------------------------------------
// Heartbeat wiring (examples/isp_pipeline/heartbeat.hpp).
//
// One slot table, one clock, one timeout counter. The ScanLoop skeleton
// consults g_slots from main's wait loops; every worker reports into a slot it
// registered inside its own start(), so the worker count and the slot count are
// the same number by construction.
// ---------------------------------------------------------------------------
uint64_t hb_now_us() { return g_pal->monotonic_ns() / kNsPerUs; }

// The non-AO worker threads start in three places (the worker block further
// down, the USB DMA worker, and the IRSC producer). Naming the count keeps the
// heartbeat assertion tied to that wiring rather than to a bare literal.
constexpr uint32_t kWorkerThreadCount = 7U;
static_assert(hb::Slots::kCapacity >= kWorkerThreadCount,
              "heartbeat slot table must cover every worker thread");

hb::Slots g_hb_slots{};
uint32_t  g_hb_timeouts = 0U;

void hb_on_timeout(const char* name, void*)
{
    ++g_hb_timeouts;
    std::printf("[hb] TIMEOUT: %s\n", name);
}

uint32_t hb_registered_slots()
{
    uint32_t n = 0U;
    g_hb_slots.ForEachSlot([&n](const char*, uint64_t, bool) { ++n; });
    return n;
}
...
int main()
{
    ...
    // Heartbeat: the clock must be registered before the first worker start(),
    // because Register() stamps the slot with the current time.
    hb::SetClock(&hb_now_us);
    hb::g_slots = &g_hb_slots;
    g_hb_slots.SetOnTimeout(&hb_on_timeout, nullptr);
    ...
    // All seven worker threads are now live (IrscWorker is a bounded producer,
    // so this is the one point where all seven heartbeat slots exist at once).
    // Slot registration happens inside each start(), before its thread_create,
    // so the count is deterministic here rather than racy.
    const uint32_t hb_slots_at_stream = hb_registered_slots();
    ...
    auto wait_txn_closed = [&recfg]() {
        // The original body returned from THIS lambda on success and fell
        // through to the failure counter on exhaustion. A plain `break` ->
        // `return false` rewrite would run the counter on success too, so the
        // outcome is carried in a flag.
        bool closed = false;
        uint32_t w = 0U;
        hb::ScanLoop(
            [&] { return w < 500U; },
    ...
    auto t37_drain = [&]() {
        // Never returns a value and nothing follows the loop, so `return` ->
        // `return false` is equivalent here.
        uint32_t w = 0U;
        hb::ScanLoop(
            [&] { return w < 5000U; },
            [&]() -> bool {
    ...
    // Heartbeat: every worker slot was registered, and none of them timed out
    // across the whole run (no false positives under real-time pacing).
    check(kWorkerThreadCount == hb_slots_at_stream,
          "heartbeat: every worker slot registered");
    check(0U == g_hb_timeouts, "heartbeat: no worker timeouts during the run");
