...
| 任务 | 产出 | 验证 |
|---|---|---|
| #32 拆分多文件 | isp_pipeline/ 14 文件，旧单文件已删；ODR 风险修复（hpp 非 inline 函数 inline 化 40+ 处） | 零警告，62 断言 ALL PASS |
| #33 MSH 监控示例 | msh_monitor_demo.cpp 1375 行：status/data/health 三层查询，const 静态命令表，场景 6 幕 + 10 次查询 | 24 断言 ALL PASS，ctest +1 |
| #35 槽位所有权 | DdrCtx SlotOwner 三态协议（写前验主/读侧 claim-release 夹 memcpy），激活 overrun_drops | 断言 52→62 增强 |
...
| #40 coact::coro | 13 头文件（TaskRegistry/Task/combinators/StackfulExecutor/scheduler 等）+ 6 测试 + rg 门禁；命名从 async 更名 coro | ctest 50/50，TSan 零 race，30 连跑稳定 |
...
| #43 BitFieldView | include/coact/bitfield.hpp 编译期位域视图 + PeriphRegCache 位域层（REG_SOUT_CTRL opcode/mode/enable 等）+ RMW 隔离断言 | worktree 产出待合并 |
| #45 vocabulary | 装入 NewType/ScopeGuard/optional/FixedFunction/function_ref/FixedString/FixedVector（newosp 移植，not_null 按用户裁定裁掉）；TaskId/TimerTaskId 手写 struct 改 NewType 别名 | 进行中（见 §5） |
...
- **深度评审**（docs/review_task_code_architecture.md v1.1 任务书）：C-1 ×1（rcGoHome 误挂 kRcSync→kRcResume 弧，会话窗口提前关闭）+ Major 7（cond_broadcast 唤醒丢失、RtThread() 16KiB 默认 ctor、constexpr 表缺 inline、裸 int、RT-Thread 线程槽清理缺口、kRcPrecheck 占位、死代码）+ Minor 6；另有 500 字精简版任务书 docs/review_task_concise.md

## 4. 文档产出

| 文档 | 内容 |
|---|---|
| docs/design_architecture_zh.md | 742 行 13 图 6 章：架构全景/层次嵌套关系/黑板五要素/硬件并发四武器/监控三层查询 |
| docs/design_consistency_avoidance_zh.md | 808 行：一致性规避设计 + 评审裁决表 |
| docs/cpp_coding_conventions_zh.md | 418 行：纯 C++17 规约 124 条 + 47 项 checklist（全体 agent 遵循） |
| docs/functional_test_report_zh.md | 124 行：测试方法 + RS500 对齐表（模拟深度分级）+ 一致性问题解决对照 |
| docs/isp_pipeline_demo_run_log.txt | 755 行：日志阅读指南 + 18 处内联中文阶段注释 |
| docs/review_task_code_architecture.md | 评审任务书 v1.1（含首轮遗留项复核表） |
| docs/review_task_concise.md | 500 字精简版评审任务书 |
...
- **构建**：make -j12 零警告零错误
- **测试**：ctest 50/50（基线 37 → 拆分 44 → coro 50）；isp_pipeline_demo 62 断言 ALL PASS；msh_monitor_demo 24 断言 ALL PASS
