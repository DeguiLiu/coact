# Design: coact 单示例系统性规避 RS500 九类一致性异常

> 本文以 `examples/isp_pipeline_demo.cpp`（单一综合示例：由 RS500 业务场景驱动的架构模型与故障注入演示，模拟深度为消息发生级）为主体，说明 coact 示例集如何以"主动对象 + 单线程事件事务"的统一架构，系统性规避 RS500 嵌入式项目复盘文档中由"不一致"引发的九类异常。每类异常均按固定结构展开：故障机理 → 架构对策 → 示例落点 → 测试证据。
>
> **证据等级声明**：本文引用的输出均来自 **host POSIX 模拟**（`build/examples/isp_pipeline_demo`），不代表 RS500 板级行为；`check()` 断言共 52 条，当前构建产物运行终局 `RESULT: ALL PASS (fails=0)`、连续 5 次运行稳定（退出码 0）、全量 ctest 37/37 通过（flash_proxy_demo 已注册）。早期版本的断言局限（Scenario A 恒真断言、Scenario B/C 日志语义、U8 修复侧未断言）已在重构中全部修复，逐项见 6.4 评审回应的"已于重构修复"标注。对评审文档（`design_consistency_avoidance_review_zh.md`）五项结论的裁决表亦见附录 6.4。
...
stateDiagram-v2
    ...
    Syncing --> Resuming : layout_version++
    Resuming --> Commit : kFirstFrame（首帧字节校验）
    Commit --> Idle : 字节==权威 → std::exchange 提交
...
**测试证据**（isp_pipeline_demo T37 UVC 阶段，当前 host 模拟输出，52 条自检断言 ALL PASS、fails=0、退出码 0）：
