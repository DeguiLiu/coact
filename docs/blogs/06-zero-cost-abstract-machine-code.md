...
上述结论的前提是"编译期结构"约束。违反约束的写法会产生显著运行期代价：
...
| 团队以 C 为主，不接受模板 | C | 语言是手段；守不住编译期结构约束，C++ 优势归零 |
...
结论：coact 采用 C++17 的决定性因素不是性能，而是类型与内存安全。性能趋同与启动不受影响消除了拒绝 C++ 的障碍；前提是守住约束——`-fno-exceptions -fno-rtti`、编译期结构、避免动态容器。
