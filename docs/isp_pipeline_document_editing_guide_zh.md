...
每份文档承担一个首要问题，不越界承担其他文档的职责。每份文档开头应列出关联文档，并说明本文件的阅读范围，读者无需在多个文件中猜测信息归属。文档增删或合并时，被删内容的首要问题应随内容迁移到存留文档，不得保留指向已删除文件的链接。

| 文档 | 首要回答的问题 | 不应承担的内容 |
|---|---|---|
| `example_isp_pipeline_design_zh.md` | 业务流程、模块职责、状态变化和协作关系；故障如何产生、如何处理、如何避免复发 | 详细测试结果、完整代码实现清单 |
| `interface_contract.md` | 模块之间的接口契约、生命周期与错误处理纪律 | 业务背景与示例讲解 |
| `cpp17_coact_usage_zh.md` | `coact::coro`（单 pthread、固定槽位、可选 ucontext）的落地用法 | 通用编码规约 |
| `cpp_coding_conventions_zh.md` | 代码是否满足现代 C++17 的可判定规约 | 模块设计与文档组织 |
| `design_nand_shared_access_zh.md` | 多 AO 共享 NAND 时如何串行化读写与互相通知 | 全量模块架构介绍 |
| `hotpath_profiling_zh.md` | 无 perf/root 权限时如何采样并定位热点 | 功能正确性结论 |
| `isp_pipeline_document_editing_guide_zh.md` | 章节上限、术语与图表表达约定、删减规则与完成检查表 | 代替设计文档解释系统原理 |

设计文档由原架构文档与原一致性文档合并而成，两者的首要问题都已并入其标题行。功能测试报告与运行日志不再作为独立文档保留，其验证要求以本文第 5、6 节的约定形式给出。
...
4. **验证入口**：只保留结果索引与断言结论；详细证据如需独立文档，在该文档开头声明归属。
...
避免只写抽象分类，不要只写"事件传播""提交边界""结构性收束"等概念，应补充动作和结果，例如：
...
中文说明之外的原始输出必须保持不变。可用以下方式核对：将整理后的日志剥离注释行，与程序原始输出逐行比较。

```bash
LOG=<整理后的运行日志文件>
awk 'BEGIN{raw=0} /^\[worker\] cmd_dma started/{raw=1} raw && $0 !~ /^【/{print}' "$LOG" \
  >/tmp/raw_without_comments.log
diff -u /tmp/isp_coro_raw.log /tmp/raw_without_comments.log
```
