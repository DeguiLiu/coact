...
示例不是被动跑流程，而是**自带验证器**：main() 末尾约 52 项 `check()` 断言逐一核对运行终态与不变量，任一失败打印 `[FAIL]` 并以非零退出码结束（`RESULT: ALL PASS` / `RESULT: FAILURES`）。断言分五组：
...
**验证意义**：分层日志使"52 项断言全 PASS"可被人工复核——任一断言失败可从结构化轨迹回溯定位。例如事务窗口自驱关闭路径，3 次 `recfg txn terminal (self)` 轨迹与对应断言一一对应；submit 拒绝缺陷修复后 rejected 计数与日志一致。
...
架构答案归结为三条结构保证：**写者唯一**（拥有者 AO）、**更新原子**（事件事务 + HSM 窗口）、**确认对等**（停稳弧 + 显式窗口）。52 项断言是"解决了"的可验证证据。
